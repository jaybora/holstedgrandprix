      ...
      $window.gapi.auth.authorize(conf,
        function(auth_result){
          if(!auth_result.error){
            ...
            console.log('Failure in authentication: %s', auth_result.error)
            loaded_q.resolve(false);
            authed_q.reject('Failure in authentication: ' + auth_result.error)
          }
      });
      return q.promise;
    };
    ...
    var load = function(name, version, custom_api_base){
      ...
      loaded_q.promise.then(function(){
        ...
        /* Load new library */
          ...
          $log.info('Loading google api: ' + name + ' api_base: ' + api_base)
